...
/// window, simulate a paste chord, restore whatever was on the clipboard before. Mirrors
/// <see cref="SelectionGrabber"/>: the paste chord depends on the target window (see
/// <see cref="InputProfile"/>) because several terminals don't treat Ctrl+V as paste.
/// </summary>
public static class TextInserter
{
    ...
    public static async Task<bool> PasteAsync(string text, IntPtr targetWindow, CancellationToken ct = default)
    {
        ...
        DebugLog.Write($"TextInserter: pasting ({chord}), length={text.Length}");
        InputSimulator.SendPasteChord(chord);
